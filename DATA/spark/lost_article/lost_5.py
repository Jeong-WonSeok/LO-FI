...
lost_df = spark.read.format("jdbc").option("url", "jdbc:mysql://j7b102.p.ssafy.io/lo-fi?serverTimezone=Asia/Seoul") \
    .option("driver", "com.mysql.jdbc.Driver").option("dbtable", "lost_article") \
...
for row in lost_df.rdd.collect():
  if(str(row.update_day) == today):
    if(index==round(cnt * 0.5) -1 ): break
    
    index = index + 1
    
    all = row.name + "/?/" + row.category + "/?/" + row.location + "/?/" + row.description 
    ...
    category = all_spell[1]
    
    location = all_spell[2]
    Pos = spellCheck.coordinate_change_dt(location)
    latitude = Pos['y']
    longitude = Pos['x']
    
    description = all_spell[3]
    
    
    conn = pymysql.connect(host='j7b102.p.ssafy.io', user='b102', password='ssafy', db='lo-fi', charset='utf8')
    cur = conn.cursor()
    sql = 'update lost_article set name = %s, category = %s, location = %s, latitude = %s, longitude = %s, description = %s where id = %s'
    cur.execute(sql, (name, category , location, latitude, longitude, description, row.id) )
    conn.commit()
    conn.close()
    ...
    for value in keyword:
      ...
      value_df = spark.createDataFrame([(str(value), None, None, row.id, None)], keyword_schema)
...
lost_df.write.option("truncate","true").jdbc(url='jdbc:mysql://j7b102.p.ssafy.io/lo-fi?serverTimezone=Asia/Seoul', table='lost_article',

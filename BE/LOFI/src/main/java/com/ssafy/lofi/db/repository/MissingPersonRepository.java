...
public interface MissingPersonRepository extends JpaRepository<MissingPerson, Long> {
    ...
    @Query(nativeQuery = true, value = "select m.* from missing_person as m " +
            "where (6371*acos(cos(radians(:lat))*cos(radians(m.latitude))*cos(radians(m.longitude)-radians(:lon))+sin(radians(:lat))*sin(radians(m.latitude)))) <= 1 order by m.date DESC")
